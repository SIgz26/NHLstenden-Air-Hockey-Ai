...
from PyQt5.QtWidgets import (
    ...
    QFrame, QFileDialog, QSlider, QSizePolicy, QComboBox, QGridLayout
)
from PyQt5.QtGui import QFont
from PyQt5.QtCore import Qt, pyqtSignal

from core.vision_engine import VisionWorker
...
class OrionDashboard(QWidget): 
    ...
    def handle_color_picked(self, norm_x, norm_y):
        ...
        if not self.vision_worker or not hasattr(self.vision_worker, 'current_hsv_frame') or self.vision_worker.current_hsv_frame is None:
            self.status_label.setText("Fout: Start eerst de video om een kleur te kiezen!")
            return

        hsv_frame = self.vision_worker.current_hsv_frame
        h_img, w_img = hsv_frame.shape[:2]
        px = max(1, min(w_img - 2, int(norm_x * w_img)))
        py = max(1, min(h_img - 2, int(norm_y * h_img)))

        # Neem 3x3 regio rond muisklik
        ...
        # Blokkeer tijdelijk signals om dubbele updates te voorkomen
        ...
        # Update de tekstlabels naast de sliders
        ...
        # EXPLICIETE SYNC NAAR CORE: Stuur de nieuwe waarden direct door naar de Worker thread!
    ...
    def init_ui(self):
        ...
        subtitle_label = QLabel("AI VISION // CONTROL CENTER")
        ...
        c1_title = QLabel("VIDEO SOURCE")
        c1_title.setObjectName("SubTitleLabel")

        self.btn_select_file = QPushButton("SELECT VIDEO")
        self.btn_select_file.clicked.connect(self.select_video_file)

        self.btn_start = QPushButton("START SCANNING")
        self.btn_start.setEnabled(False)
        self.btn_start.clicked.connect(self.toggle_vision)

        self.status_label = QLabel("Status: Geen bestand geselecteerd")
        self.status_label.setWordWrap(True)

        c1_layout.addWidget(c1_title)
        c1_layout.addWidget(self.btn_select_file)
        ...
        self.combo_view.addItems(["Single View (Tracking)", "Dual View (Tracking + Mask)"])
        ...
        # --- Card 2: Camera Displays & Player Controls ---
        card2 = QFrame()
        ...
        self.btn_pause.clicked.connect(self.toggle_pause_video)

        self.slider_timeline = QSlider(Qt.Horizontal)
        self.slider_timeline.setEnabled(False)
        self.slider_timeline.sliderMoved.connect(self.seek_video)
        ...
        self.feed_primary = ClickableVideoLabel("Primary Feed")
        ...
        self.feed_secondary = QLabel("Secondary Feed (Mask)")
        ...
        self.feed_secondary.setScaledContents(False)
    ...
    def sync_vision_settings(self):
        self.update_color_preview()
        if self.vision_worker and self.vision_worker.isRunning():
            # Stuur direct de actuele sliderwaarden door naar de VisionWorker thread
            self.vision_worker.update_hsv(
    ...
    def select_video_file(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(
            self, "Selecteer Videoclip", "", "Video Files (*.mp4 *.avi *.mov *.mkv)", options=options
        )
        if file_path:
            self.selected_video_path = file_path
            filename = os.path.basename(file_path)
            self.status_label.setText(f"Geladen: {filename}")
    ...
    def toggle_vision(self):
        ...
        if self.vision_worker is None or not self.vision_worker.isRunning():
            self.vision_worker = VisionWorker(self.selected_video_path)
            
            self.vision_worker.frame_processed.connect(self.update_feed)
            self.vision_worker.status_signal.connect(self.update_status)
            self.vision_worker.position_changed.connect(self.update_timeline)
    ...
    def toggle_pause_video(self):
        if self.vision_worker and self.vision_worker.isRunning():
            is_paused = self.vision_worker.toggle_pause()
            self.btn_pause.setText("PLAY" if is_paused else "PAUSE")

    def seek_video(self, position):
        if self.vision_worker and self.vision_worker.isRunning():
            self.vision_worker.set_position(position)

    def update_timeline(self, current_frame, total_frames):
        self.slider_timeline.blockSignals(True)
        self.slider_timeline.setRange(0, total_frames)
